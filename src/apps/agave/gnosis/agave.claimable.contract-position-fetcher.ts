...
import { AgaveClaimableContractPositionHelper } from '../helper/agave.claimable.contract-position-helper';
...
export class GnosisAgaveClaimableContractPositionFetcher implements PositionFetcher<ContractPosition> {
  constructor(
    @Inject(AgaveClaimableContractPositionHelper)
    private readonly agaveClaimableContractPositionHelper: AgaveClaimableContractPositionHelper,
  ) {}

  async getPositions() {
    return this.agaveClaimableContractPositionHelper.getTokens({
      appId,
      groupId,
      network,
